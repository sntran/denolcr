...
async function router(request: Request): Promise<Response> {
  ...
  if (method === "GET") {
    ...
    try {
      ...
      const { size, nchunks }: Metadata = JSON.parse(
 ...
 */
function formatName(name: string, index: number, format: string): string {
  return format.replace("*", name).replace(/([#])\1+/g, (hashes) => {
    return `${index}`.padStart(hashes.length, "0");
  });
}

async function upload(url: string | URL, { headers, body }: RequestInit) {
